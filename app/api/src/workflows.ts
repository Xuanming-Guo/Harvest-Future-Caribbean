  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.traceStep.create({
      data: {
        ...
        provenance: Provenance.MODEL_PREDICTED,
        summary: !promisable
          ? `The ${methodLabel} (${prediction.modelVersion}) put q10 at ${prediction.q10} kg, but the crop is ${current.status.toLowerCase().replaceAll("_", " ")} so nothing can be promised from it.`
          : isReadyStatus(current.status)
            ? `The ${methodLabel} (${prediction.modelVersion}) put q10 at ${prediction.q10} kg; ${committedQuantity} kg is committed, leaving ${availableToPromise} kg ATP available today.`
            : `The ${methodLabel} (${prediction.modelVersion}) put q10 at ${prediction.q10} kg; ${committedQuantity} kg is committed, leaving ${availableToPromise} kg promisable from ${promisableFrom!.toISOString().slice(0, 10)}.`,
