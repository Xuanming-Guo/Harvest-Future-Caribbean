...
| `INTER_ISLAND_COMMITMENT_PROPOSED` | Harvest proposes filling part of an order from another in-scope island | commitment, order, origin and destination islands, published link and ports, quantity, synthetic cost in both currencies | No world mutation and nothing binding; the sailing cannot be booked until every `INTER_ISLAND_COMMITMENT` approval is granted |
| `INTER_ISLAND_COMMITMENT_APPROVED` | Every counterparty has approved a cross-island commitment | commitment, order, approver count, bound time | The commitment may now be booked onto its published sailing |
| `MARITIME_SHIPMENT_SCHEDULED` | An approved cross-island commitment is booked onto a published sailing | shipment, commitment, order, three legs, synthetic capacity, synthetic customs checkpoint | Record the consignment; the physical schedule, capacity, customs behaviour and outcome stay synthetic |
